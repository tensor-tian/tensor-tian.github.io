...
export async function transformImportedCode(
  tree: Root,
  file?: { history?: string[]; cwd?: string },
) {
  console.log("transformImportedCode, file", file)
  const nodes: Code[] = []
      ...
      const newLines = await Promise.all(
        lines.map(async (line) => {
          if (line.startsWith("!from ")) {
            ...
            return await readFile(codepath, mdxPath, file?.cwd, range)
          }
          return line
        }),
      )
...
Looks like node "fs" and "path" modules are not available.`
  ...
  const absoluteCodepath =
    cwd && externalCodePath.startsWith("@/")
      ? path.resolve(cwd, "public", externalCodePath.slice(2))
      : path.resolve(dir, externalCodePath)
